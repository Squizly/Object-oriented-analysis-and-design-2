...
from visitors import ExcelExportVisitor, TxtExportVisitor
...
class App(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Университет | Паттерн Visitor")
    ...
    def setup_ui(self):
        ...
        self.sidebar.grid_rowconfigure(5, weight=1) # Пустое пространство для прижима логов вниз
        ...
        self.create_menu_btn(self.sidebar, "📥 Загрузить (Excel)", self.load_db, "#2563EB", "normal")
        self.create_menu_btn(self.sidebar, "💾 Сохранить (Excel)", self.save_db, "#D97706", "normal")

        # Секция Паттерна
        ctk.CTkLabel(self.sidebar, text="ЭКСПОРТ (VISITOR)", font=("Segoe UI", 12, "bold"), text_color=TEXT_MUTED, anchor="w").pack(fill="x", padx=20, pady=(30, 10))
        self.create_menu_btn(self.sidebar, "📊 Экспорт в Excel", self.visitor_excel, "#059669", "normal")
        self.create_menu_btn(self.sidebar, "📝 Экспорт в TXT", self.visitor_txt, "#0891B2", "normal")
        ...
        # ==================== ОСНОВНОЙ КОНТЕНТ (ПРАВО) ====================
        self.main_view = ctk.CTkFrame(self, fg_color="transparent")
        self.main_view.grid(row=0, column=1, sticky="nsew", padx=20, pady=20)
        self.main_view.grid_rowconfigure(1, weight=1)
        self.main_view.grid_columnconfigure(0, weight=4) # Список (шире)
        self.main_view.grid_columnconfigure(1, weight=3) # Редактор (уже)
        ...
        # --- СПИСОК (ЦЕНТР-ЛЕВО) ---
        self.list_frame = ctk.CTkScrollableFrame(self.main_view, fg_color=PANEL_BG, corner_radius=15, scrollbar_button_color="#3F3F46")
        self.list_frame.grid(row=1, column=0, sticky="nsew", padx=(0, 10))

        # --- РЕДАКТОР (ЦЕНТР-ПРАВО) ---
        ...
        # Скрываемый фрейм редактора
        self.edit_form = ctk.CTkFrame(self.edit_container, fg_color="transparent")

    # ==================== МЕТОДЫ СОЗДАНИЯ UI ====================

    def create_menu_btn(self, parent, text, command, color, state):
    ...
    def create_stat_card(self, parent, col, title, value, color=TEXT_MAIN):
        ...
        lbl_title = ctk.CTkLabel(card, text=title, font=("Segoe UI", 12, "bold"), text_color=TEXT_MUTED)
        lbl_title.place(relx=0.1, rely=0.2)
    ...
    def load_db(self, silent=False):
        try:
            ...
            self.log("[DB] Данные успешно загружены.")
        except Exception as e:
            if not silent: messagebox.showerror("Ошибка", f"Файл не найден или поврежден:\n{e}")

    def save_db(self):
        try:
            DataManager.save_data(self.univer)
            self.log("[DB] Успешно сохранено в Excel.")
            messagebox.showinfo("Сохранено", "Изменения записаны в файл!")
        except Exception as e:
    ...
    def update_list(self):
        ...
        for person in self.univer.persons:
            ...
            # Функция для передачи клика с текста на саму карточку
            def on_label_click(event, p=person):
                self.open_editor(p)

            # Иконка
            lbl_icon = ctk.CTkLabel(card, text=icon, font=("Segoe UI", 24))
            lbl_icon.place(relx=0.03, rely=0.2)
            lbl_icon.bind("<Button-1>", on_label_click) # Делаем текст кликабельным
            ...
            # Информация справа
            info_text = f"Курс {person.course}" if is_student else f"{person.experience} лет стажа"
    ...
    def open_editor(self, person):
        ...
        if is_student:
            fields.update({"studentID": "ID Студента", "course": "Курс", "GPA": "Средний балл (GPA)"})
        else:
            fields.update({"post": "Должность", "experience": "Стаж (лет)"})

        # Сетка для полей ввода (чтобы было красиво в два столбца)
    ...
    def apply_edits(self):
        ...
        self.update_list()
        self.update_dashboard()
        self.log(f"[EDIT] Изменен профиль: {self.selected_person.fullName}")

    # ==================== ПАТТЕРН VISITOR ====================

    def visitor_excel(self):
        if not self.univer.persons: return messagebox.showwarning("Пусто", "Нет данных для экспорта.")
        visitor = ExcelExportVisitor()
        self.univer.accept(visitor)
        path, count = visitor.save_file()
        self.log(f"[VISITOR] Excel: Экспортировано {count} записей.")
        messagebox.showinfo("Успех", f"Excel успешно сгенерирован!\n{path}")

    def visitor_txt(self):
        if not self.univer.persons: return messagebox.showwarning("Пусто", "Нет данных для экспорта.")
        visitor = TxtExportVisitor()
        self.univer.accept(visitor)
        path, count = visitor.save_file()
        self.log(f"[VISITOR] TXT: Экспортировано {count} записей.")
        messagebox.showinfo("Успех", f"TXT лог успешно сгенерирован!\n{path}")
